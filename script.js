...
const activeFrameText = $("activeFrameText");
...
function bindEvents() {
  ...
  doneEditBtn.onclick = closeEditMode;
...
function renderTemplateOptions() {
  ...
  CONFIG.templates.filter((template) => template.layout === layoutId).forEach((template) => {
    const option = document.createElement("option");
...
async function loadTemplate(id) {
  ...
  if (photoAreas.length < getLayout().photoCount) photoAreas = generateFallbackAreas();
}
...
function detectMarkerAreas(img, width, height) {
  ...
  for (let y = 0; y < height; y += step) {
    for (let x = 0; x < width; x += step) {
      ...
      if (area.w > 80 && area.h > 80) areas.push(area);
    }
  }
  areas.sort((a, b) => (Math.abs(a.x - b.x) > 30 ? a.x - b.x : a.y - b.y));
...
function flood(startX, startY, width, height, data, visited, target, step) {
  ...
  while (queue.length) {
    ...
    [[x + step, y], [x - step, y], [x, y + step], [x, y - step]].forEach(([nx, ny]) => {
...
function isMarkerPixel(data, index, target) {
  return Math.abs(data[index] - target.r) <= CONFIG.tolerance &&
...
function generateFallbackAreas() {
  ...
  const layout = getLayout();
  const p = 90;
  if (id === "2") return [{ x: p, y: 140, w: layout.width - p * 2, h: 480 }, { x: p, y: 700, w: layout.width - p * 2, h: 480 }];
  if (id === "3") return [{ x: p, y: 120, w: layout.width - p * 2, h: 430 }, { x: p, y: 610, w: layout.width - p * 2, h: 430 }, { x: p, y: 1100, w: layout.width - p * 2, h: 430 }];
  if (id === "4") return [{ x: p, y: 100, w: layout.width - p * 2, h: 390 }, { x: p, y: 560, w: layout.width - p * 2, h: 390 }, { x: p, y: 1020, w: layout.width - p * 2, h: 390 }, { x: p, y: 1480, w: layout.width - p * 2, h: 390 }];
  if (id === "6") return [{ x: p, y: 90, w: layout.width - p * 2, h: 330 }, { x: p, y: 470, w: layout.width - p * 2, h: 330 }, { x: p, y: 850, w: layout.width - p * 2, h: 330 }, { x: p, y: 1230, w: layout.width - p * 2, h: 330 }, { x: p, y: 1610, w: layout.width - p * 2, h: 330 }, { x: p, y: 1990, w: layout.width - p * 2, h: 330 }];
  if (id === "4v2") return [{ x: 90, y: 170, w: 470, h: 560 }, { x: 90, y: 820, w: 470, h: 560 }, { x: 640, y: 170, w: 470, h: 560 }, { x: 640, y: 820, w: 470, h: 560 }];
  if (id === "6v2") return [{ x: 90, y: 150, w: 470, h: 470 }, { x: 90, y: 690, w: 470, h: 470 }, { x: 90, y: 1230, w: 470, h: 470 }, { x: 640, y: 150, w: 470, h: 470 }, { x: 640, y: 690, w: 470, h: 470 }, { x: 640, y: 1230, w: 470, h: 470 }];
  return [];
}
...
async function renderPreview() {
  ...
  if (!templateImage) drawDefaultTemplate(ctx, getLayout(), areas);
  for (let i = 0; i < photos.length; i++) {
    const img = await loadImage(photos[i]);
    if (img && areas[i]) drawPhotoToArea(ctx, img, areas[i], edits[i] || getDefaultEdit());
  }
  if (templateImage) drawTemplateWithoutMarkers(ctx, templateImage, canvasWidth, canvasHeight);
  if (editMode) drawActiveFrame(ctx, areas[activeFrame]);
  updateStatus();
}
...
function drawTemplateWithoutMarkers(ctx, img, width, height) {
  ...
  const target = hexToRgb(CONFIG.markerColor);
  for (let i = 0; i < data.length; i += 4) if (isMarkerPixel(data, i, target)) data[i + 3] = 0;
...
function drawActiveFrame(ctx, area) {
  if (!area) return;
  ctx.save();
  ctx.lineWidth = 10;
...
function updateStatus() {
  ...
  statusBox.innerHTML = `SESSION<br><br>Layout<br><b>${layout.name}</b><br><br>Template<br><b>${currentTemplate ? currentTemplate.name : "Default"}</b><br><br>Photo<br><b>${photos.length}</b> / <b>${layout.photoCount}</b><br><br>Peace<br><b>${peaceEnabled ? "ON" : "OFF"}</b><br><br>Camera<br><b>${stream ? "CONNECTED" : "OFF"}</b>`;
  ...
  if (done) {
    ...
    if (stream) sessionActions.style.display = "grid";
  }
}
...
function updateEditUI() {
  editBtn.textContent = editMode ? "SELESAI EDIT" : "EDIT FOTO";
  editPanel.classList.toggle("active", editMode);
  previewCanvas.classList.toggle("editing", editMode);
  activeFrameText.textContent = photos[activeFrame] ? `Frame: ${activeFrame + 1}` : "Frame: -";
}
...
function frameFromPointer(event) {
  ...
  for (let i = 0; i < areas.length; i++) {
    const area = areas[i];
    if (x >= area.x && x <= area.x + area.w && y >= area.y && y <= area.y + area.h) return i;
  }
  return activeFrame;
}

function startDrag(event) {
  if (!editMode || !photos.length) return;
...
function moveDrag(event) {
  if (!editMode || !dragging || !photos[activeFrame]) return;
...
function wheelZoom(event) {
  if (!editMode || !photos[activeFrame]) return;
  event.preventDefault();
